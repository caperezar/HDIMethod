function DSf = DS(f,bdy)
% Adjoint double layer operator computed using HOSS
    ...
    for a=1:nElem
        ...
        for b=1:nElem
            ...
            if a==b
                ...
                v = DSfar(f(b),bdy(a),bdy(b));
            end
            
            DSf(a).vec = DSf(a).vec+v.vec;
...
function DSf = DSself(f,bdy)

    DSf = Density;
    DSf = DSf.cctor(bdy,f.dim);
            ...
            for q=1:ni
                ...
                for j=1:bdy.nPat
                    ...
                    for d=1:f.dim
                        ...
                        DSf.vec(indX,d) = DSf.vec(indX,d) + dGdn*((F-Q).*Jcb)+d2Gdn2*(P.*Jcb);
    ...
    DSf.vec = DSf.vec+f.vec/2;
    
end

function DSf = DSfar(f,bdy_evl,bdy_int)

    DSf = Density;
    DSf = DSf.cctor(bdy_evl,f.dim);
    ...
    for i = 1:bdy_evl.nPat   
        ...
        for j=1:bdy_int.nPat
            ...
            dGdn = -(R1.*Nx1+R2.*Nx2+R3.*Nx3)./(4*pi*R.^3);
            ...
            if ~isempty(ind_evl_aux)
                NNear = size(ind_evl_aux,1);                
                ind_aux =[ind_aux;ind_evl_aux repmat(i,NNear,1) loc(ind_evl_aux) repmat(j,NNear,1)]; 
            end
            %%            
    ...
    if ~isempty(ind)
        ...
        for i = 1:Nnear
            ...
            for j=1:bdy_int.nPat
                ...
                outVec(indX) = outVec(indX) + dGdn*((F-Q).*Jcb)+d2Gdn2*(P.*Jcb);
